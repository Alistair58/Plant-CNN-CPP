...
class CNN : CnnUtils{
    ...
    public CNN(float LR,Dataset dataset,boolean restart){
        numNeurons = new int[] {1920,960,47};
        numMaps = new int[] {3,30,60,120};//includes the result of pooling (except final pooling)
        mapDimens = new int[]    {128,64,32,16};
        kernelSizes = new int[]  {5,3,3};  //0 represents a pooling layer, the last one is excluded
        strides = new int[]      {2,2,2,4}; //pooling strides are included
        padding = true;
        verbose = false;
        ...
        this.LR = LR;
        this.activations = new float[numNeurons.length][];
        for(int l=0;l<numNeurons.length;l++){
            activations[l] = new float[numNeurons[l]];
        }
        this.maps = new float[numMaps.length][][][];
    ...
    public CNN(CNN template,float LR,Dataset dataset) {
    ...
    public String forwards(int[][][] imageInt){
        ...
        //Final pooling 
        float[][][] pooled = new float[numMaps[numMaps.length-1]][][];
        for(int i=0;i<numMaps[numMaps.length-1];i++){
            pooled[i] = maxPool(maps[numMaps.length-1][i],strides[strides.length-1],strides[strides.length-1]);
            for(int y=0;y<pooled[i].length;y++){
                System.arraycopy(pooled[i][y], 0, activations[0], i*pooled[i].length*pooled[i][y].length+y*pooled[i][y].length, pooled[i][y].length);
        ...
        for(int l=0;l<weights.length;l++){
            for(int i=0;i<numNeurons[l+1];i++){
                ...
                activations[l+1][i] = activations[l+1][i] + weights[l][i][activations[l].length]; //add bias
                if(l!=weights.length-1){ //We'll softmax the last layer and so relu is unnecessary
                    activations[l+1][i] = leakyRelu(activations[l+1][i]);
                }
            }
        }
        activations[activations.length-1] = softmax(activations[activations.length-1]);
        float largestActivation = activations[activations.length-1][0];
        int result = 0;
        for(int i=1;i<numNeurons[numNeurons.length-1];i++){
            if(activations[activations.length-1][i]>largestActivation){
                largestActivation = activations[activations.length-1][i];
                result = i;
            }
        }
        if(verbose){
            System.out.println(Arrays.toString(activations[activations.length-1]));
            System.out.println("Forwards took "+(System.currentTimeMillis()-startTime)+"ms");
        }
        return d.plantNames.get(result);
    }

    public void backwards(int[][][] imageInt,String answer){ //adds the gradient to its internal gradient arrays
        ...
        float[][][][] dcDxs = new float[numMaps.length-2][][][];
        //No dcDxs for first or last (last goes straight into the MLP)
        for(int l=0;l<numMaps.length-2;l++){
            if(kernelSizes[l+1]==0){
        ...
        for(int l=numMaps.length-2;l>0;l--){ //>0 is due to the input dimens being included in numMaps and -2 as we've already done the last layer
            if(kernelSizes[l-1]==0){
    ...
    private void mlpBackwards(float[][] dcDzs){
        for(int l=weights.length-1;l>=0;l--){
            for(int i=0;i<numNeurons[l+1];i++){
                ...
                //bias
                weightsGrad[l][i][activations[l].length] += dcDzs[l+1][i];
            }
        }
    }

    private void convBackwards(float[][][][] dcDxs, int l,boolean padding){
        int lSub1 = l-1;
        int lSub2 = l-2;
                    ...
                    for(int k=0;k<kernelSize;k++){ //For each element in the kernel (k,j)
                        ...
                        for(int y=yStart;y<yEnd;y+=thisStride){  //For every pixel in the previous layer (x,y) which then corresponds to one in the current (x-k,y-j)
                            for(int x=xStart;x<xEnd;x+=thisStride){
                                if(!floatCmp(dcDxs[lSub1][i][thisY][thisX],0f)){ //yes, l-1 is correct (dcDxs only has numMaps.length-2 layers)
    ...
    private void finalPoolingConvBackwards(float[][] dcDzs,float[][][][] dcDxs,boolean padding){
        for(int i=0;i<numMaps[numMaps.length-1];i++){ //for each final map
            int prevDimens = mapDimens[numMaps.length-2];
            int currDimens = mapDimens[numMaps.length-1];
            int kernelSize = kernelSizes[kernelSizes.length-1];
            int kernelRadius = (int) Math.floor(kernelSize/2);
            int poolStride = strides[strides.length-1];
            int thisStride = strides[strides.length-2];
            int poolWidth = mapDimens[numMaps.length-1]/strides[strides.length-1];
            int poolArea = poolWidth*poolWidth;
            int mlpRegion = i*poolArea; //Saving a few multiplications
            for(int prevMapI=0;prevMapI<numMaps[numMaps.length-2];prevMapI++){
                for(int j=0;j<kernelSize;j++){
                    for(int k=0;k<kernelSize;k++){ //For each element in the kernel (k,j)
                        //Add up all the activations that it sees
                        float sum = 0f;
                        ...
                        for(int y=yStart;y<yEnd;y+=thisStride){  //For every pixel in the previous layer (x,y) which then corresponds to one in the current (thisX,thisY)
                            for(int x=xStart;x<xEnd;x+=thisStride){ //Derivatve of the corresponding pixel in the next (backwards) layer
                                ...
                                if(floatCmp(maps[maps.length-1][i][thisY][thisX],activations[0][mlpIndex]) && !done[mlpSubIndex]){ //only the max element has a derivative
                                    done[mlpSubIndex] = true;
                                    //In the first MLP layer a=relu(x) where x is the max activation pixel from pooling
                                    dcDxs[dcDxs.length-1][prevMapI][y][x] += dcDzs[0][mlpIndex] * kernels[kernels.length-1][i][prevMapI][j][k] ;//*kernel weight
                                    sum+=maps[maps.length-2][prevMapI][y][x]*dcDzs[0][mlpIndex]; //The activation of the previous layer * the correct derivative from pooling
                                }
                                thisX++;
                            }
                            thisX = 0;
                        ...
                        kernelsGrad[kernels.length-1][i][prevMapI][j][k] += sum;
                    }
                }
            }
            float biasSum = 0f;
            ...
            for(int y=0;y<currDimens;y++){
                for(int x=0;x<currDimens;x++){
                    ...
                    mlpIndex = mlpRegion + mlpSubIndex;
                    //Bias has to be here as otherwise it would count the same pixels multiple times
                    if(floatCmp(maps[maps.length-1][i][y][x],activations[0][mlpIndex]) && !done[mlpSubIndex]){
                        done[mlpSubIndex] = true;
                        biasSum += dcDzs[0][mlpIndex]; //Bias deriv = cost deriv * relu deriv * 1 (only 1 bias term in each new pixel expression
                    }
                }
            }
            kernelsGrad[kernels.length-1][i][numMaps[numMaps.length-2]-1][kernelSize-1][kernelSize] += biasSum;
        }
    }

    private void poolingConvBackwards(float[][][][] dcDxs, int l,boolean padding){
