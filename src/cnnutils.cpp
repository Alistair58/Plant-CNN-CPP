...
class CnnUtils {
    ...
    protected:
            ...
            d3 loadWeights(bool loadNew){
                if(loadNew){
                    float[][][] lWeights = new float[numNeurons.size() -1][][];
    ...
    public:    
        void applyGradients(){ //(and reset gradients)
            float adjustedGrad;
        ...
        void applyGradients(CNN[] cnns){ //(and reset gradients)
        ...
        void resetWeights() {
            for (int i = 0; i < weights.size(); i++) { //layer
                for (int j = 0; j < weights[i].size(); j++) { //neurone
                    float stdDev = (float) Math.sqrt((float)2/(weights[i][j].size()-1));
